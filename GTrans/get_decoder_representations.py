...
def local_test_from_file_from_small_track1():
    # Read from the Files
    input_files = ["/path/to/flores101_dataset/devtest-code/valid.hr",
                   "/path/to/flores101_dataset/devtest-code/valid.sr"]
    sourceLanguages = ['hrv', 'srp']
    targetLanguages = ['srp', 'hrv']
    output_file = "/path/to/flores101_dataset/dynalab.translation"
    data = []
...
def local_test_from_file_small_track2():
    # Read from the Files
    input_files = ["/path/to/flores101_dataset/devtest-code/valid.id",
                   "/path/to/flores101_dataset/devtest-code/valid.en",
                   "/path/to/flores101_dataset/devtest-code/valid.id"]
...
def local_test_from_file():
    # Read from the Files
    input_files = ["/path/to/flores101_dataset/devtest-code/valid.en"] * 10
    sourceLanguages = ['en'] * 10
    targetLanguages = "fr cs de fi lv et ro hi tr gu".split()
    PATH = "/path/to/data/repr/"
    for input_file, sourceLanguage, targetLanguage in zip(input_files, sourceLanguages, targetLanguages):
        data = read_from_file(input_file, sourceLanguage, targetLanguage)
        bin_data = b"\n".join(json.dumps(d).encode("utf-8") for d in data)
        torchserve_data = [{"body": bin_data}]
        manifest = {"model": {"serializedFile": "/path/to/model/wmt10/XLMT/avg4_8.pt"}}
        system_properties = {"model_dir": ".", "gpu_id": 0}
