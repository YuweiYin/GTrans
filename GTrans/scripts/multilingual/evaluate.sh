...
cat "${input_file}" | python scripts/spm_encode.py --model "${spm_model}" | \
  python scripts/multilingual/truncate.py | python fairseq_cli/interactive.py "${data_bin}" \
    --path "${model_file}" \
    ...
    --remove-bpe=sentencepiece --no-progress-bar | grep -P "^H" \
    | cut -f 3- > "${log_dir}/translation.${src}-${tgt}"
