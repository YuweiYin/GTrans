...
class ContextCropDataset(TruncateDataset):
    ...
    def __init__(self, dataset, truncation_length, start_token, seed=1):
        super().__init__(dataset, truncation_length)
        self.seed = seed
        self.epoch = 0
    ...
    def __getitem__(self, index):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            if excess > 0:
                ...
                item = item[start_idx : start_idx + self.truncation_length]
                if item[0] != self.start_token and self.start_token is not None:
                    item = torch.cat([self.start_token, item[:-1]])
            return item
