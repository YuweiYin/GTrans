...
class XLMTEncoder(TransformerEncoderFromPretrainedInfoXLM):
...
class XLMTDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.drop_adapter = getattr(args, "drop_adapter", -1)
        self.adapter_method = getattr(args, "adapter_method", "all")
    ...
    def extract_features_scriptable(
        ...
        if self.use_adapter:
            if torch.rand((1,)) > self.drop_adapter or not self.training:
                tgt_lang_id = self.get_lang_id(tgt_lang_id) - 1
                if self.adapter_method == "all":
                    x = self.adapter[tgt_lang_id](x)
                elif self.adapter_method == "high" and self.langs[tgt_lang_id] in self.high_langs:
                    x = self.adapter[tgt_lang_id](x)
