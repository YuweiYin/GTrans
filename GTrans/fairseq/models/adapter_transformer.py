...
def expand_embedding_matrix(state, encoder):
    if state['model']['encoder.embed_tokens.weight'].size(0) < encoder.embed_tokens.weight.size(0):
        ...
        state['model']['encoder.embed_tokens.weight'] = torch.cat([state['model']['encoder.embed_tokens.weight'], state['model']['encoder.embed_tokens.weight'].new(offset, embed_dim).fill_(0)], dim=0)
...
class AdapterTransformerModel(TransformerModel):
    ...
    def build_encoder(cls, args, tgt_dict, embed_tokens):
        return AdapterEncoder(args, tgt_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return AdapterDecoder(args, tgt_dict, embed_tokens)

    def __init__(self, args, encoder, decoder):
        super().__init__(args, encoder, decoder)
        if hasattr(args, "pretrained_mt_checkpoint") and os.path.exists(args.pretrained_mt_checkpoint):
            mt_loaded_state_dict = upgrade_mt_state_for_adapter_model(
                state_dict=self.state_dict(),
                pretrained_mt_checkpoint=args.pretrained_mt_checkpoint,
                encoder=self.encoder
            )
            logger.info("Loading pretrained mt model from {0}".format(args.pretrained_mt_checkpoint))
            self.load_state_dict(mt_loaded_state_dict, strict=True)
        else:
            logger.info("Can not Load pretrained mt model !")
        ...
        **kwargs
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.use_adapter:
                ...
                x = self.adapter[idx][src_lang_id](x)
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.use_adapter:
                ...
                x = self.adapter[idx][tgt_lang_id](x)
...
@register_model_architecture(
    "adapter_transformer", "xlmt_adapter"
)
...
@register_model_architecture(
    "adapter_transformer", "adapter_transformer_big"
)
