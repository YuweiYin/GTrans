...
@register_model("two_stage_sparse_transformer")
class TwoStageSparseModel(TransformerFromPretrainedInfoXLMModel):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument(
            "--adapter-method",
            type=str,
            choices=["soft_hard", "gumbel_softmax"]
        )
        parser.add_argument(
            "--adapter-dim",
            type=int,
        ...
        parser.add_argument(
            "--reset-lang2adapter-dict",
            action="store_true",
        )

    @classmethod
    def build_encoder(cls, args, tgt_dict, embed_tokens):
        return TwoStageSparseEncoder(args, tgt_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TwoStageSparseDecoder(args, tgt_dict, embed_tokens)
    ...
    def upgrade_xlmr_state_for_decoder(
            self, state_dict: Dict[str, Any], xlmr_state_dict: str, num_layers: int, prefix="decoder.sentence_encoder.", new_prefix="decoder."
    ) -> Dict[str, Any]:
        for key in xlmr_state_dict.keys():
            if 'layers' in key and int(key.split('.')[3]) > 2 * num_layers - 1:
                continue
            ...
            if 'lm_head' not in key:
                if 'in_proj_weight' in key:
                    ...
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 0:
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_weight', 'q_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}')] = q
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_weight', 'k_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}')] = k
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_weight', 'v_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}')] = v
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_weight', 'q_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}').replace('self_attn', 'encoder_attn')] = q
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_weight', 'k_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}').replace('self_attn', 'encoder_attn')] = k
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_weight', 'v_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}').replace('self_attn', 'encoder_attn')] = v
                elif 'in_proj_bias' in key:
                    q, k, v = xlmr_state_dict[key].chunk(3, dim=0)
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 0:
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_bias', 'q_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}')] = q
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_bias', 'k_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}')] = k
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_bias', 'v_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}')] = v
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_bias', 'q_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}').replace('self_attn', 'encoder_attn')] = q
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_bias', 'k_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}').replace('self_attn', 'encoder_attn')] = k
                        state_dict[key.replace(prefix, new_prefix).replace('in_proj_bias', 'v_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}').replace('self_attn', 'encoder_attn')] = v
                elif 'fc1' in key:
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 0:
                        state_dict[key.replace(prefix, new_prefix).replace('fc1', 'fc3').replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                elif 'fc2' in key:
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 0:
                        state_dict[key.replace(prefix, new_prefix).replace('fc2', 'fc4').replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                elif 'final_layer_norm' in key:
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 0:
                        state_dict[key.replace(prefix, new_prefix).replace('final_layer_norm', 'ffn_layer_norm').replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                elif 'self_attn.out_proj' in key:
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 1:
                        state_dict[key.replace(prefix, new_prefix).replace('self_attn.out_proj', 'encoder_attn.out_proj').replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                elif 'self_attn.k_proj' in key or 'self_attn.v_proj' in key or 'self_attn.q_proj' in key:
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 1:
                        state_dict[key.replace(prefix, new_prefix).replace('self_attn', 'encoder_attn').replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                elif 'self_attn_layer_norm' in key:
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 1:
                        state_dict[key.replace(prefix, new_prefix).replace('self_attn_layer_norm', 'encoder_attn_layer_norm').replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                    else:
                        state_dict[key.replace(prefix, new_prefix).replace(f'.{i_layer}', f'.{i_layer // 2}')] = xlmr_state_dict[key]
                elif 'emb_layer_norm' in key:
                    state_dict[key.replace(f'{prefix}emb_layer_norm', f'{new_prefix}layernorm_embedding')] = xlmr_state_dict[key]
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if "decoder.sentence_encoder.embed_tokens.weight" in state_dict: #XLM-R State
            ...
            if getattr(self.args, "init_decoder_only", False):
                cur_state_dict = self.upgrade_xlmr_state_for_decoder(cur_state_dict, xlmr_state_dict=state_dict, num_layers=self.args.decoder_layers, prefix="decoder.sentence_encoder.", new_prefix="decoder.")
                logger.info(f"Loading XLM-R for Decoder of {self.args.arch} (Two FFns in Single Decoder Block)")
            state_dict.clear()
    ...
    def forward(
        ...
        decoder_out = self.decoder(
            ...
            tgt_lang_embedding = self.encoder.embed_tokens(src_tokens[0,0]),  #the first language token (descending order)
            epoch=extra_args["epoch"] if "epoch" in extra_args.keys() else 1
        )
        return decoder_out
...
class AdapterLayer(nn.Module):
    def __init__(self, args):
        ...
        self.adapter_dim = getattr(args, "adapter_dim", self.embed_dim)
...
class TwoStageSparseEncoder(TransformerEncoderFromPretrainedInfoXLM):
    def __init__(self, args, dictionary, embed_tokens):
        super().__init__(args, dictionary, embed_tokens)
        if getattr(args, 'freeze_encoder_layers', 0) > 0:
            for param in self.layers[: args.freeze_encoder_layers].parameters():
...
class TwoStageSparseDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        super().__init__(args, dictionary, embed_tokens, no_encoder_attn)
        if not getattr(args, "init_decoder_only", False):
            return

        self.gumbel_softmax_temperature = getattr(args, "gumbel_softmax_temperature", math.sqrt(embed_tokens.embedding_dim))
        self.use_adapter = getattr(args, "use_adapter", False)
        self.swap_adapter = getattr(args, "swap_adapter", 0)
        self.hard_adapter = getattr(args, "hard_adapter", 1)
        self.start_hard_epoch = getattr(args, "start_hard_epoch", 1)
        self.end_soft_epoch = getattr(args, "end_soft_epoch", 15)
        self.langs = args.langs
        self.lang_id = args.lang_id
        assert hasattr(args, "high_langs") and hasattr(args, "low_langs"), "Please set high- and low-resource langauges"
        self.high_langs = args.high_langs.split(',')
        self.low_langs = args.low_langs.split(',')
        self.high_lang_id = {lang: self.lang_id[lang] for lang in self.high_langs}
        self.high_lang_tensor = torch.LongTensor(list(self.high_lang_id.values()))
        self.adapter_num = getattr(args, "adapter_num", len(self.high_langs))
        self.adapter_method = getattr(args, "adapter_method", "soft_hard")
        if self.use_adapter:
            ...
            self.adapter.extend(
                nn.ModuleList(
                    [
                        AdapterLayer(args)
                        for _ in range(self.adapter_num + 1)
                    ])
            )
            self.gate = nn.Linear(embed_tokens.embedding_dim, self.adapter_num, bias=True)
        else:
            self.adapter = None
        args.pretrained_infoxlm_checkpoint = getattr(args, "pretrained_infoxlm_checkpoint", "")
        args.pretrained_gpt_checkpoint = getattr(args, "pretrained_gpt_checkpoint", "")
        if os.path.exists(args.pretrained_infoxlm_checkpoint) or os.path.exists(args.pretrained_gpt_checkpoint):
            if args.variant == 'addffn':
                ...
                logger.info("Loading decoder from {0}".format(args.pretrained_gpt_checkpoint))
        ...
        if getattr(args, 'freeze_decoder_layers', 0) > 0:
            for param in self.layers[: args.freeze_decoder_layers].parameters():
                param.requires_grad = False
        ...
        if getattr(args, 'freeze_embedding', False):
            for param in self.embed_tokens.parameters():
                param.requires_grad = False
        #
        self.training_phase = getattr(args, "training_phase", "phase-1")
        self.register_buffer('lang2adapter', torch.FloatTensor([0] * len(self.high_langs)))
        self.lang2adapter_dict = None


    def convert_lang2adapter_dict_gpu2cpu(self):
        for key in self.lang2adapter_dict.keys():
            self.lang2adapter_dict[key] = self.lang2adapter_dict[key].cpu().tolist()


    def update_lang2adapter(self):
        lang_soft_weights = torch.softmax(self.gate(self.embed_tokens(self.high_lang_tensor)), dim=1)
        self.lang2adapter = torch.argmax(lang_soft_weights, dim=1)
        self.lang2adapter_dict = {self.high_langs[i]: self.lang2adapter[i] for i in range(len(self.lang2adapter.cpu().tolist()))}
        return lang_soft_weights


    def build_decoder_layer(self, args, no_encoder_attn=False):
        if args.variant == 'addffn' or args.variant == 'gpt-addffn':
            layer = XLMTAddFFN(args, no_encoder_attn)
        elif args.variant == 'transformer':
    ...
    def forward(
        ...
        x, extra = self.extract_features(
            ...
            tgt_lang_embedding=tgt_lang_embedding,
            epoch=epoch
        )
    ...
    def update_lang2adapter_dict(self):
        return {self.high_langs[i]: int(self.lang2adapter[i].cpu().tolist()) for i in range(len(self.lang2adapter.cpu().tolist()))}


    def swap_adapter_forward(self, x, swap_adapter_prob, hard_or_soft_adapter_prob, soft_weights, src_lang_id, tgt_lang_id, epoch):
        lang_similarity = torch.tensor([0]).type_as(x)
        lang_soft_weights = torch.tensor([0]).type_as(x)
        if self.training_phase == "phase-1":
            if self.langs[tgt_lang_id] in self.high_langs and swap_adapter_prob > self.swap_adapter or not self.training:
                if hard_or_soft_adapter_prob < self.hard_adapter or not self.training or epoch > self.end_soft_epoch:
                    if epoch > self.end_soft_epoch:
                        if self.lang2adapter_dict is None:
                            self.lang2adapter_dict = self.update_lang2adapter_dict()
                        adapter_id = self.lang2adapter_dict[self.langs[tgt_lang_id]]
                        x = self.adapter[adapter_id](x)
                    else:
                        x = self.hard_adapter_forward(x, soft_weights)
                else:
                    x = self.soft_adapter_forward(x, soft_weights)
                    lang_soft_weights = self.update_lang2adapter()
                    lang_similarity = torch.matmul(lang_soft_weights, lang_soft_weights.transpose(0, 1)).sum()
                    #lang_similarity = torch.triu(torch.matmul(lang_soft_weights, lang_soft_weights.transpose(0, 1)))
                    #lang_similarity = lang_similarity.sum()
            else:
                x = self.adapter[-1](x)
        elif self.training_phase == "phase-2":
            if self.langs[tgt_lang_id] in self.high_langs: # self.langs[src_lang_id] in self.high_langs and self.langs[tgt_lang_id] in self.high_langs
                if self.lang2adapter_dict is None:
                    if getattr(self.args, "reset_lang2adapter_dict", False):
                        logger.info("Reset the lang2adapter_dict using the language symbol embedding !")
                        self.update_lang2adapter()
                        self.convert_lang2adapter_dict_gpu2cpu()
                    else:
                        self.lang2adapter_dict = self.update_lang2adapter_dict()
                        #self.lang2adapter_dict = {self.high_langs[i]: int(self.lang2adapter[i].cpu().tolist()) for i in range(len(self.lang2adapter.cpu().tolist()))}
                    logger.info("lang2adapter_dict: {}".format(self.lang2adapter_dict))
                adapter_id = self.lang2adapter_dict[self.langs[tgt_lang_id]]
                x = self.adapter[adapter_id](x)
            else:
                x = self.adapter[-1](x)
            ...
            NotImplementedError
        return x, lang_soft_weights, lang_similarity


    def soft_adapter_forward(self, x, soft_weights):
        x = [soft_weights[adapter_id] * self.adapter[adapter_id](x) for adapter_id in range(len(self.adapter) - 1)]
        x = torch.stack(x, dim=0).sum(0)
        return x


    def hard_adapter_forward(self, x, soft_weights):
        adapter_id = torch.argmax(soft_weights)
        x = self.adapter[adapter_id](x)
        return x


    def gumbel_softmax_forward(self, x, tgt_lang_id, tgt_lang_embedding, epoch):
        if self.langs[tgt_lang_id] in self.high_langs:
            ...
            #     lang_similarity = lang_similarity.sum()
            # else:
            #     lang_soft_weights = None
            #     lang_similarity = None
            if self.training_phase == "phase-1":
                lang_soft_weights = self.update_lang2adapter()
                logits = self.gate(tgt_lang_embedding)

            if self.training and self.training_phase == "phase-1" and torch.rand(1,) > self.hard_adapter and epoch <= self.end_soft_epoch:
                soft_weights = F.gumbel_softmax(logits, tau=self.gumbel_softmax_temperature, hard=True)
                x = self.soft_adapter_forward(x,  soft_weights)
                lang_similarity = torch.triu(torch.matmul(lang_soft_weights, lang_soft_weights.transpose(0, 1)))
                lang_similarity = lang_similarity.sum()
            else:
                if self.training_phase == "phase-1":
                    ...
                    if self.lang2adapter_dict is None:
                        if getattr(self.args, "reset_lang2adapter_dict", False):
                            ...
                            self.lang2adapter_dict = {self.high_langs[i]: int(self.lang2adapter[i].cpu().tolist()) for i in range(len(self.lang2adapter.cpu().tolist()))}
                        logger.info(self.lang2adapter_dict)
                    soft_weights = None
                    adapter_id = self.lang2adapter_dict[self.langs[tgt_lang_id]]
                    x = self.adapter[adapter_id](x)
                lang_soft_weights = None
            ...
            x = self.adapter[-1](x)
            soft_weights, lang_soft_weights, lang_similarity = None, None, None
        return x, soft_weights, lang_soft_weights, lang_similarity
    ...
    def extract_features_scriptable(
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        self.high_lang_tensor = self.high_lang_tensor.type_as(prev_output_tokens)
        if src_lang_id is not None:
            src_lang_id = self.get_lang_id(src_lang_id) - 1
        ...
        if self.use_adapter:
            swap_adapter_prob = torch.rand((1,))
            hard_or_soft_adapter_prob = torch.rand((1,))
            if tgt_lang_embedding is None:
                lang_idx = torch.LongTensor([self.lang_id[self.langs[tgt_lang_id]]]).type_as(prev_output_tokens)
                tgt_lang_embedding = self.embed_tokens(lang_idx.squeeze(0))
            soft_weights = torch.softmax(self.gate(tgt_lang_embedding), dim=0)
            #x = self.swap_adapter_forward(x, swap_adapter_prob=swap_adapter_prob, hard_or_soft_adapter_prob=hard_or_soft_adapter_prob, tgt_lang_id=tgt_lang_id, soft_weights=soft_weights, epoch=epoch)
        ...
        if self.use_adapter:
            if self.adapter_method == "soft_hard":
                x, lang_soft_weights, lang_similarity = self.swap_adapter_forward(x, swap_adapter_prob=swap_adapter_prob, hard_or_soft_adapter_prob=hard_or_soft_adapter_prob, src_lang_id=src_lang_id, tgt_lang_id=tgt_lang_id, soft_weights=soft_weights, epoch=epoch)
            elif self.adapter_method == "gumbel_softmax":
                x, soft_weights, lang_soft_weights, lang_similarity = self.gumbel_softmax_forward(x, tgt_lang_id, tgt_lang_embedding, epoch)
        ...
        return x, {"attn": [attn], "inner_states": inner_states, "lang_soft_weights": lang_soft_weights, "lang_similarity": lang_similarity}
...
@register_model_architecture(
    "two_stage_sparse_transformer", "two_stage_sparse_transformer"
)
def base_architecture(args):
    ...
    args.swap_adapter = getattr(args, "swap_adapter", 0)
    args.hard_adapter = getattr(args, "hard_adapter", 1)
    args.start_hard_epoch = getattr(args, "start_hard_epoch", 1)
    args.end_soft_epoch = getattr(args, "end_soft_epoch", 15)
    args.training_phase = getattr(args, "training_phase", "phase-1")
    args.gumbel_softmax_temperature = getattr(args, "gumbel_softmax_temperature", math.sqrt(args.decoder_embed_dim))



@register_model_architecture(
    "two_stage_sparse_transformer", "two_stage_sparse_transformer_large"
)
def base_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
