...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def train_step(
        ...
        with torch.autograd.profiler.record_function("forward"):
            loss, sample_size, logging_output = criterion(model, sample, epoch=epoch)
        if ignore_grad:
            loss *= 0
    ...
    def valid_step(self, sample, model, criterion):
        ...
        #     logging_output["src_lang"] = self.data_manager.lang_dict.symbols[logging_output["src_lang_id"]]
        #     logging_output["tgt_lang"] = self.data_manager.lang_dict.symbols[logging_output["tgt_lang_id"]]
        #     logging_output["lang_pair"] = "{}->{}".format(logging_output["src_lang"], logging_output["tgt_lang"])
        #     del logging_output["src_lang_id"]
        #     del logging_output["tgt_lang_id"]
        return loss, sample_size, logging_output
