...
class DistributedTrainingConfig(FairseqDataclass):
    ...
    ddp_backend: DDP_BACKEND_CHOICES = field(
        default="no_c10d", metadata={"help": "DistributedDataParallel backend"}
    )
...
class CheckpointConfig(FairseqDataclass):
    ...
    start_validate_epoch: int = field(
        default=1, metadata={"help": "start validate from the  N-th epoch"}
    )
    start_save_epoch: int = field(
        default=1, metadata={"help": "start validate from the  N-th epoch"}
    )
